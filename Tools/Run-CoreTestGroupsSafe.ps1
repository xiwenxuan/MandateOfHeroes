...
if ($PrepareOnly) {
    ...
    $listResult = Invoke-BoundedProcess `
        ...
        -ArgumentList @($resolvedProject, $binaryDirectory, "--list") `
        -WorkingDirectory $resolvedProject `
        -LogDirectory $runRoot `
...
if (-not $AggregateOnly) {
    ...
    $filter = "exact:" + ($expected -join ';')
    $execution = Invoke-BoundedProcess `
        -Name "core-tests-group-$GroupIndex" `
        -FilePath $coreRunnerPath `
        -ArgumentList @($resolvedProject, $binaryDirectory, $filter) `
        -WorkingDirectory $resolvedProject `
        -LogDirectory $groupDirectory `
        -HardTimeoutSeconds $TimeoutSeconds
    $actual = @(Get-Content -LiteralPath $execution.Stdout |
        Where-Object { $_ -match '^PASS (.+)$' } |
        ForEach-Object { $Matches[1] } |
        Sort-Object -Unique)
    $failures = @(Get-Content -LiteralPath $execution.Stdout |
        Where-Object { $_ -match '^FAIL ' })
    $summary = Select-String -LiteralPath $execution.Stdout `
        -Pattern '^RESULT passed=(\d+) failed=(\d+)$'
    $missing = @($expected | Where-Object { $_ -notin $actual })
    $unexpected = @($actual | Where-Object { $_ -notin $expected })
    if ($null -eq $summary -or $failures.Count -gt 0 -or
        $missing.Count -gt 0 -or $unexpected.Count -gt 0 -or
        [int]$summary.Matches[0].Groups[1].Value -ne $expected.Count -or
        [int]$summary.Matches[0].Groups[2].Value -ne 0) {
        throw (
    ...
    $result = [ordered]@{
        ...
        passed = $actual.Count
        failed = 0
        stdoutPath = $execution.Stdout
        stderrPath = $execution.Stderr
        tests = $actual
    }
