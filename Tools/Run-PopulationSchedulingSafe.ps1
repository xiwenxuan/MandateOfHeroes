...
param(
    [string]$ProjectPath,
    [ValidateRange(1000, 1000000)][int]$PersonCount = 10000,
    [long]$Seed = 14000015,
...
function Invoke-BoundedProcess {
    ...
    $metadata = [ordered]@{
        schema_version = "m15.p2.process-run.v1"; name = $Name; pid = $process.Id
...
$workingRoot = Join-Path $resolvedProject "tmp\m15-p2"
...
foreach ($source in $sources) { if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "P2 source is missing: $source" } }
$executablePath = Join-Path $binaryRoot "PopulationScheduling.exe"
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $workingRoot "m15-p2-$PersonCount-$stamp.json" }
$resolvedOutput = [System.IO.Path]::GetFullPath($OutputPath)
$allowedOutputRoot = [System.IO.Path]::GetFullPath($workingRoot).TrimEnd([System.IO.Path]::DirectorySeparatorChar) + [System.IO.Path]::DirectorySeparatorChar
if (-not $resolvedOutput.StartsWith($allowedOutputRoot, [System.StringComparison]::OrdinalIgnoreCase)) { throw "P2 output must remain under $workingRoot" }
$workspace = Join-Path $workingRoot "work-$stamp"
$arguments = @(
...
$run = Invoke-BoundedProcess -Name "schedule-$PersonCount" -FilePath $executablePath -ArgumentList $arguments -WorkingDirectory $resolvedProject `
    -LogDirectory $logRoot -HardTimeoutSeconds $TimeoutSeconds
if (-not (Test-Path -LiteralPath $resolvedOutput -PathType Leaf)) { throw "P2 scheduling exited without a result file: $resolvedOutput" }
...
if ($result.status -cne "passed" -or -not $result.cross_backend_determinism.passed -or @($result.backends).Count -ne 3) {
    throw "P2 result does not contain three passing deterministic backends: $resolvedOutput"
}
Write-Host "Scheduling metadata: $($run.MetadataPath)"
Write-Host "Scheduling result: $resolvedOutput"
Write-Host "RESULT m15-p2=passed backends=3 people=$PersonCount deterministic=true"
