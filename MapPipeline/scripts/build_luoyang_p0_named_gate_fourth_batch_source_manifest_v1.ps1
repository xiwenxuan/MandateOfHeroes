...
function Get-FileRecord {
    ...
    if (-not (Test-Path -LiteralPath $fullPath -PathType Leaf)) {
        throw "Required named-gate fourth-batch source file is missing: $fullPath"
    }
    return [ordered]@{
        path = $normalized
...
if ($catalog.ProfileCount -ne 4 -or $catalog.Profiles.Count -ne 4 -or
    $catalog.StatusId -ne
        "LUOYANG_P0_NAMED_GATE_FOURTH_BATCH_USER_ACCEPTED_FBX_SOURCE_VALIDATED_FINAL_ART_ACTIVATED_V1" -or
    $catalog.ReviewDecisionStatusId -ne
        "user_review.luoyang-p0-named-gate-fourth-batch.accepted.v1" -or
    $catalog.UserReviewDecisionRecordId -ne
        "decision.luoyang-p0-named-gate-fourth-batch.accepted.2026-08-27.v1" -or
    ...
    throw "Named-gate fourth-batch accepted catalog gate is invalid."
}

$expectedOrders = @(11, 12, 13, 14)
...
if (@(Compare-Object $expectedOrders $actualOrders).Count -ne 0) {
    throw "Named-gate fourth-batch selection no longer contains review orders 11,12,13,14."
}
...
$assetPaths.AddRange([string[]]@(
    "Assets/Scripts/Mandate.Domain/LuoyangP0NamedGateFourthBatchState.cs",
    "Assets/Scripts/Mandate.Persistence/LuoyangP0NamedGateFourthBatchSource.cs",
    "Assets/Editor/Mandate.Editor/LuoyangP0NamedGateFourthBatchArtBuilder.cs",
    "Assets/Editor/Mandate.Editor/LuoyangP0NamedGateFourthBatchFbxExporter.cs",
    ...
    "Assets/Scripts/Mandate.Presentation/HanWorldNaturalMapController.cs",
    "Assets/Scripts/Mandate.Presentation/ExplicitStrategicCellMapV1.cs",
    $catalogPath,
    "Assets/Resources/Art/Han/Luoyang/P0Batch4/Gumen.prefab",
    "Assets/Resources/Art/Han/Luoyang/P0Batch4/Jinmen.prefab",
    "Assets/Resources/Art/Han/Luoyang/P0Batch4/Kaiyangmen.prefab",
    "Assets/Resources/Art/Han/Luoyang/P0Batch4/Maomen.prefab",
...
foreach ($assetPath in ($assetPaths | Sort-Object)) {
    if (-not $uniqueAssets.Add($assetPath)) {
        throw "Duplicate named-gate fourth-batch source path: $assetPath"
    }
    $sourceFiles.Add((Get-FileRecord $assetPath))
    $sourceFiles.Add((Get-FileRecord ($assetPath + ".meta")))
}
if ($sourceFiles.Count -ne 56) {
    throw "Expected 56 source and metadata files, found $($sourceFiles.Count)."
}
...
foreach ($profile in ($catalog.Profiles | Sort-Object ReviewOrder)) {
    if (-not $profile.ArtistPrefabPresent -or -not $profile.FinalArtApproved -or
        ...
        throw "Invalid accepted profile status: $($profile.CandidateId)"
    }
    $fbxRecord = Get-FileRecord ([string]$profile.ArtistFbxTargetPath)
    if ($fbxRecord.length -lt 1024) {
        throw "Named-gate fourth-batch FBX is unexpectedly small: $($profile.ArtistFbxTargetPath)"
    }
