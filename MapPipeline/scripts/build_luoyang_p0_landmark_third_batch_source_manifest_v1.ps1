...
function Get-FileRecord {
    ...
    if (-not (Test-Path -LiteralPath $fullPath -PathType Leaf)) {
        throw "Required third-batch source file is missing: $fullPath"
    }
    return [ordered]@{
        path = $normalized
...
if ($catalog.ProfileCount -ne 4 -or $catalog.Profiles.Count -ne 4 -or
    $catalog.StatusId -ne
        "LUOYANG_P0_LANDMARK_THIRD_BATCH_USER_ACCEPTED_FBX_SOURCE_VALIDATED_FINAL_ART_ACTIVATED_V1" -or
    $catalog.ReviewDecisionStatusId -ne
        "user_review.luoyang-p0-landmark-third-batch.accepted.v1" -or
    $catalog.UserReviewDecisionRecordId -ne
        "decision.luoyang-p0-landmark-third-batch.accepted.2026-08-27.v1" -or
    ...
    throw "Third-batch accepted final-activation catalog gate is invalid."
}

$expectedOrders = @(6, 7, 8, 9)
...
if (@(Compare-Object $expectedOrders $actualOrders).Count -ne 0) {
    throw "Third-batch selection no longer contains review orders 6,7,8,9."
}
...
$assetPaths.AddRange([string[]]@(
    "Assets/Scripts/Mandate.Domain/LuoyangP0LandmarkThirdBatchState.cs",
    "Assets/Scripts/Mandate.Persistence/LuoyangP0LandmarkThirdBatchSource.cs",
    "Assets/Editor/Mandate.Editor/LuoyangP0LandmarkThirdBatchArtBuilder.cs",
    "Assets/Editor/Mandate.Editor/LuoyangP0LandmarkThirdBatchFbxExporter.cs",
    ...
    $catalogPath,
    "Assets/Resources/Art/Han/Luoyang/P0Batch3/Lingtai.prefab",
    "Assets/Resources/Art/Han/Luoyang/P0Batch3/Taicang.prefab",
    "Assets/Resources/Art/Han/Luoyang/P0Batch3/Arsenal.prefab",
    "Assets/Resources/Art/Han/Luoyang/P0Batch3/ZhuolongGarden.prefab",
...
foreach ($assetPath in ($assetPaths | Sort-Object)) {
    if (-not $uniqueAssets.Add($assetPath)) {
        throw "Duplicate third-batch source path: $assetPath"
    }
    $sourceFiles.Add((Get-FileRecord $assetPath))
    $sourceFiles.Add((Get-FileRecord ($assetPath + ".meta")))
}
if ($sourceFiles.Count -ne 60) {
    throw "Expected 60 source and metadata files, found $($sourceFiles.Count)."
}
...
foreach ($profile in ($catalog.Profiles | Sort-Object ReviewOrder)) {
    ...
    if ($fbxRecord.length -lt 1024) {
        throw "Third-batch FBX is unexpectedly small: $($profile.ArtistFbxTargetPath)"
    }
