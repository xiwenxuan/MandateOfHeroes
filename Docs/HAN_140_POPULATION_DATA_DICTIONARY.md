...
| 模型版本 | `han140.p<阶段>[.<批次>].v<版本>`；当前使用`han140.p1.batch1.v1`至`han140.p1.batch4.v1` |
...
| `model_version` | 使用`han140.p<阶段>[.<批次>].v<版本>`格式；当前按录入批次使用`han140.p1.batch1.v1`至`han140.p1.batch4.v1` |
...
当前P1前四批已录入汉、司隶、幽州、冀州层级及27条人口记录，其中司隶七郡尹、
冀州九郡国、幽州十一郡与属国已经完成。辽东属国原文户口为空；辽东郡人口、
玄菟郡户数和辽东属国估算分别保留原始值与修正值。司隶河南尹、弘农郡的现代
ODS异录只作说明，不覆盖卷029原典值。稳定地理、映射和游戏地点交叉表仍为空，
分别保留至P2、P3；不得把三个区域完成描述为全国P1完成。
