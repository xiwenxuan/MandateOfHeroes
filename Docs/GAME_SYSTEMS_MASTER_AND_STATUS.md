...
| 永久人口 | 已有底座（M14村庄切片、M15-P6、M20-P0、M21-P0） | 200—500人生活闭环、累计1000万冷热档案、5000万全部在世基础调度证据、V7正式分区人口包、关注热计划、人物仓储访问合同与按分区增量检查点；未驻留人口尚未从`WorldState.People`移出 |
...
| 超大人口存储 | 已有底座与实验（M15-P6、M21-P0） | 正式程序集已有稳定分区、完整代际、只重写脏人物所属分区的增量检查点、清单校验和冷热扩展合同；旅行、关系、任务已开始使用人物访问层；完整运行时外置与游戏负载仍未完成 |
...
汇总缓存。M15-P6已建立V7共存迁移和正式分区人口包，M21-P0已建立统一人物仓储访问
合同，并迁移旅行、关系和任务三个活跃人物系统。其余模拟系统仍有大量直接访问
...
和混合方案，M15-P6把分区二进制正式接入为V7侧车人口包；M21-P0新增稳定脏人物集合和
按受影响分区写回的增量检查点。尚未迁移的模拟系统、出生增量、变更日志压缩、代际清理、
...
触发底座。M21-P0已经完成首批人物仓储访问合同、旅行/关系/任务迁移和按分区增量检查点。
当前从尚未完成的依赖继续：

1. 继续把人口、家庭、生产、教育、军事等模拟系统分批迁移到人物仓储访问层，并接入出生增量；
