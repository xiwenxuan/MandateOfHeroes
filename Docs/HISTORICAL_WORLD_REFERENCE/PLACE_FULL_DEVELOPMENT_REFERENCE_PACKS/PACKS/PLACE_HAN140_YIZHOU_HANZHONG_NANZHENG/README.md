...
本目录是 `place.han140.yizhou.hanzhong.nanzheng` 的统一开发参考包，开发档位为 **T3**，由旧标签 `D4` 无损映射而来；既有波次 **WAVE_2** 未改变。

- 参考完整度：`FULL_READY_WITH_MODELED_GAPS`
- 运行时实现：`NOT_STARTED`
- 空间存在模式：`PERMANENT_SETTLEMENT`
