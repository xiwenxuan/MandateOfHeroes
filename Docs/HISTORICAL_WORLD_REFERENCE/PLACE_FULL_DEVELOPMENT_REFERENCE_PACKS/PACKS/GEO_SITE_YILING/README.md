...
本目录是 `geo.site.yiling` 的统一开发参考包，开发档位为 **T2**，由旧标签 `D3` 无损映射而来；既有波次 **WAVE_2** 未改变。

- 参考完整度：`FULL_READY_WITH_UNKNOWNS`
- 运行时实现：`NOT_STARTED`
- 空间存在模式：`PERMANENT_GEOGRAPHIC_SITE|EVENT_DEPENDENT_COMPLEX|BATTLEFIELD_REGION`
