...
{
    public static class LuoyangP0LandmarkThirdBatchFbxExporter
    {
        public const string RevisionId =
            "luoyang.p0.landmark-third-batch.fbx-source.v1";
        ...
        [MenuItem("Mandate/Luoyang/Export P0 Landmark Third Batch FBX V1")]
        public static void ExportFromMenu()
        {
            var report = ExportAll();
            Debug.Log("Luoyang P0 landmark third-batch FBX exported: " +
                         ...
                         item => item.ReviewOrder))
            {
                ...
                if (AssetDatabase.LoadAssetAtPath<GameObject>(prefabPath) == null)
                    throw new FileNotFoundException(
                        "Third-batch prefab is missing.", prefabPath);
                ...
                try
                {
                    ...
                    if (group == null || group.GetLODs().Length != 3 ||
                        ...
                        throw new InvalidDataException(
                            "Third-batch prefab must have three populated LODs: " +
                            prefabPath + ".");
                ...
                if (length < 1024)
                    throw new InvalidDataException(
                        "Third-batch FBX is unexpectedly small: " + fbxPath +
                        ".");
        ...
        private static void AddAnchorMarkers(GameObject root,
            LuoyangP0LandmarkThirdBatchProfile profile, out Mesh mesh,
            out Material material)
        {
            mesh = new Mesh { name = "__P0_BATCH3_ANCHOR_MARKER_MESH" };
            mesh.vertices = new[]
            {
                Vector3.zero,
            ...
            {
                name = "__P0_BATCH3_ANCHOR_MARKER"
            };
            ...
            foreach (var anchor in profile.Anchors)
            {
                ...
                if (transform == null)
                    throw new InvalidDataException(
                        "Third-batch prefab anchor is missing: " +
                        anchor.AnchorId + ".");
        ...
        private static LuoyangP0LandmarkThirdBatchCatalog LoadCatalog()
        {
            ...
            return JsonUtility.FromJson<LuoyangP0LandmarkThirdBatchCatalog>(
                       File.ReadAllText(path)) ??
                   throw new InvalidDataException(
                       "Third-batch catalog could not be parsed.");
        }
            ...
            LuoyangP0LandmarkThirdBatchCatalog catalog)
        {
            if (catalog.ProfileCount !=
                ...
                throw new InvalidDataException(
                    "Third-batch FBX export requires four accepted source prefabs.");
        }
        ...
        private static string NormalizeAndValidatePath(string path)
        {
            ...
            if (!normalized.StartsWith(SourceRoot + "/",
                ...
                normalized.Contains("../"))
                throw new InvalidDataException(
                    "Third-batch FBX must remain under " + SourceRoot + ".");
            ...
            if (!fullPath.StartsWith(fullRoot,
                    StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException(
                    "Third-batch FBX path escaped its source root.");
            return normalized;
        }
